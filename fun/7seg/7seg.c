...
static void count_chars(char const *msg, int msg_len, int *nb_stdchar,
                        int *nb_spechar)
{
    int pos = 0;
    char c;
...
static void draw_rect(char **dest, int x1, int x2, int y1, int y2,
                      char filler)
{
    int x, y;
    for (y = y1; y <= y2; y++)
...
static void draw_repr(char **dest, int segment, unsigned int pos,
                      unsigned int height, unsigned int width, char filler)
{
    int thickness =
        MIN(height / THICKNESS_DIVIDER_H, width / THICKNESS_DIVIDER_W);
...
#define BORDER_BOTTOM (height-1)
    if (segment & SEG_TOP)
        draw_rect(dest, BORDER_LEFT, BORDER_RIGHT, BORDER_TOP,
                  BORDER_TOP + thickness, filler);
    if (segment & SEG_MIDDLE)
        draw_rect(dest, BORDER_LEFT, BORDER_RIGHT,
                  middle_line - thickness + thickness / 2,
                  middle_line + thickness / 2, filler);
    if (segment & SEG_BOTTOM)
        draw_rect(dest, BORDER_LEFT, BORDER_RIGHT,
                  BORDER_BOTTOM - thickness, BORDER_BOTTOM, filler);
    if (segment & SEG_TOPLEFT)
        draw_rect(dest, BORDER_LEFT, BORDER_LEFT + thickness, BORDER_TOP,
                  middle_line, filler);
    if (segment & SEG_BOTLEFT)
        draw_rect(dest, BORDER_LEFT, BORDER_LEFT + thickness, middle_line,
                  BORDER_BOTTOM, filler);
    if (segment & SEG_TOPRIGHT)
        draw_rect(dest, BORDER_RIGHT - thickness, BORDER_RIGHT, BORDER_TOP,
                  middle_line, filler);
    if (segment & SEG_BOTRIGHT)
        draw_rect(dest, BORDER_RIGHT - thickness, BORDER_RIGHT,
                  middle_line, BORDER_BOTTOM, filler);
#undef BORDER_LEFT
#undef BORDER_RIGHT
#undef BORDER_TOP
#undef BORDER_BOTTOM
