...
void update_window(seven_seg_size *window, char ***clock_text_ptr) {
    ...
    *clock_text_ptr = malloc(window->height * sizeof(char*));
    if(!*clock_text_ptr) goto alloc_fail;
    for(cur_line = 0; cur_line < window->height; cur_line++) {
        (*clock_text_ptr)[cur_line] = malloc(window->width);
        if(!(*clock_text_ptr)[cur_line]) goto alloc_fail;
    }
    /* clear screen */
    printf("\033[2J");
    return;
...
void display_time(seven_seg_size *window, char **clock_text) {
    ...
    if((errno = seven_seg_plot(clock_text, timestr, '#', 'o', *window, &used)))
        err(43, "plotting");
...
int main() {
    ...
    for(;;) {
        if(poll(&polled_fd, 1, -1 /* block forever */) < 0 && errno != EINTR)
            err(2, "polling");
